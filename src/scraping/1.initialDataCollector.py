...
with open('schemes_data.json', 'r', encoding='utf-8') as file:
    data = json.load(file)

# Dictionary to store cleaned output
cleaned_schemes = {}

# Loop through each slug and extract required information
for slug in setofslags:
    try:
        # Skip if scheme data is None or missing
        scheme_data = data.get(slug, {}).get('pageProps', {}).get('schemeData', {}).get('en')
        if not scheme_data:
            print(f"[!] Skipping slug '{slug}': No valid scheme data found.")
            continue

        cleaned_schemes[slug] = {
            "Scheme Name": scheme_data['basicDetails'].get('schemeName', 'N/A'),
        "Ministries/Departments": scheme_data['basicDetails'].get('nodalMinistryName', {}).get('label', 'N/A'),
        "Target Beneficiaries": [b.get('label', 'N/A') for b in scheme_data['basicDetails'].get('targetBeneficiaries', [])],
        "Eligibility Criteria": scheme_data.get('eligibilityCriteria', {}).get('eligibilityDescription_md', 'N/A').strip(),
        "Description & Benefits": (
            scheme_data['schemeContent'].get('briefDescription', 'N/A').strip() + "\n\n" +
            scheme_data['schemeContent'].get('benefits_md', 'N/A').strip()
        ),
        "Application Process": scheme_data.get('applicationProcess', [{}])[0].get('process_md', 'N/A').strip(),
        "Tags": scheme_data['basicDetails'].get('tags', [])
        }

    except Exception as e:
        print(f"[!] Unexpected error for slug '{slug}': {e}")

# Save the cleaned data to 'mostclean.json'
with open('mostclean2.json', 'w', encoding='utf-8') as outfile:
    json.dump(cleaned_schemes, outfile, indent=4, ensure_ascii=False)

print("✅ Cleaned scheme data saved to 'mostclean.json'")
